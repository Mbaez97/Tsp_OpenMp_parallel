...
Individual crossoverInj(Individual a, Individual b)
{
	...
	for (int n = 0; n < CITIES; n++)
		ab.route[n] = -1;
	for (int n = x; n <= y; n++)
		ab.route[n] = a.route[n];

	for (int n = 0; n <= q; n++)
	{
		if (!inList(ab, b.route[n]))
			c.route[n] = b.route[n];
	}

	for (int h = q + 1; h < (y - x); h++)
		c.route[h] = a.route[h];
...
void mutateW(Individual &a)
{
	...
	for (int i = 0; i < MUTATEW_POINTS; i++)
	{
		...
		if (fitness(candidate) < fitness(a))
			a = candidate;
	}
}
...
void evolve(Population &p)
{
	...
	while (g < GENERATIONS)
	{
		...
		printInd(p.pop[0]);
		cout << " | GEN: " << g << "  | F=" << fitness(p.pop[0]) << endl
			 << endl;
